	...
	remoteReadStream().then(resRemoteReadStream => {
		getAudioMetadata(resRemoteReadStream).then(res => {
			...
			if (file.metadata.stereo === 'true') {
				...
				// terminate function if Agent Search Word was not found
				if (file.metadata.agentsearchword === undefined) {
					let safErrorAgent = 'Agent Search Word was not found in file.metadata'
			...
			let pubSubObj = {
				...
				'agentid': file.metadata.agentid === undefined ? 'undefined' : file.metadata.agentid,
				'date': Date(Date.now()),
				...
				'agentsearchword': file.metadata.agentsearchword === undefined ? 'undefined' : file.metadata.agentsearchword,
				'duration': duration, //get value from fluent-ffmpeg
				'stereo': file.metadata.stereo === undefined ? 'undefined' : file.metadata.stereo,
				'agentchannel': file.metadata.agentchannel === undefined ? 'undefined' : file.metadata.agentchannel
			};
